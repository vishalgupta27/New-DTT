...
class login : AppCompatActivity() {
    ...
    fun logincheck() {
        if (eMail!!.text.toString().isEmpty() || !isValidEmail(eMail!!.text.toString())) {
            Toast.makeText(this, "Enter valid Email address and Password", Toast.LENGTH_SHORT)
                .show()
        } else {
            // API Calls Start
                ...
                override fun onResponse(call: Call<Map<Any, Any>?>, response: Response<Map<Any, Any>?>) {
                    try {
                        if (response.isSuccessful && response.body() != null) {
                            ...
                            val gson = Gson()
                            val objrespose  = gson.fromJson(gson.toJson(response.body()), LoginRequestModel::class.java)
                            ...
                            Toast.makeText(this@login, "Response null aa raha hai", Toast.LENGTH_SHORT).show()
                        }
